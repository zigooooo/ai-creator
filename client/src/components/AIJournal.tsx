...
export const AIJournal: React.FC<Props> = ({ events }) => {
  return (
    <div className="glass-card" style={{ padding: '1.25rem', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <h3 style={{ fontSize: '1rem', fontWeight: 800, marginBottom: '0.85rem', color: '#ffffff', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', display: 'flex', flexDirection: 'column', gap: '0.6rem', paddingRight: '0.2rem' }}>
        {events.length === 0 ? (
          <div style={{ fontSize: '0.8rem', color: '#a1a1aa' }}>No system events logged yet.</div>
        ) : (
          events.slice().reverse().map((evt) => (
            <div key={evt.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', color: '#a1a1aa', fontSize: '0.7rem' }}>
                <span style={{ fontWeight: 700, color: '#ffffff' }}>{evt.agentName || 'System'}</span>
              ...
              {evt.payload?.action && (
                <div style={{ color: '#e2e8f0', fontSize: '0.75rem', marginTop: '0.15rem' }}>
                  {evt.payload.action}
                </div>
              )}
            </div>
